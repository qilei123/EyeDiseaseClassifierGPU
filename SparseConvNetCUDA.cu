  ...
  for (int rep=1;rep<=nReps;++rep) {
    ...
    for (int i=0;i<dataset.pictures.size();++i) {
      ...
      for (int j=0;j<nTop;j++){
		  //std::cout<<"dataset.pictures[i]->label--------:"<<dataset.pictures[i]->label<<std::endl;
		  //std::cout<<"predictions[j]-------:"<<predictions[j]<<std::endl;
...
void SparseConvNetCUDA::processDatasetRepeatTest(SpatiallySparseDataset &dataset, int batchSize, int nReps,float *scores, std::string predictionsFilename,std::string header,std::string confusionMatrixFilename)
{
	multiplyAddCount=0;
	 auto start=std::chrono::system_clock::now();
	 std::vector<std::vector<int  > > votes(dataset.pictures.size());
