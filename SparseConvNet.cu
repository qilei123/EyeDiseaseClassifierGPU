...
void SparseConvNet::processDatasetRepeatTest(SpatiallySparseDataset &dataset, int batchSize, int nReps,float *scores, std::string predictionsFilename,std::string header,std::string confusionMatrixFilename){
  cnn->processDatasetRepeatTest(dataset,batchSize,nReps,scores,predictionsFilename,header,confusionMatrixFilename);
}
